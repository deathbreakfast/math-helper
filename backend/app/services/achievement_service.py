...
class AchievementService:
    ...
    def ensure_achievements(user: User, metrics: dict[str, Any] | None = None) -> list[Achievement]:
        ...
        if all_achievements:
            _debug_print(f"[ACHIEVEMENT DEBUG] Awarded {len(all_achievements)} achievement(s) from config: {[a.code for a in all_achievements]}")
            print(f"[ACHIEVEMENT INFO] Awarded {len(all_achievements)} general achievement(s): {[a.title for a in all_achievements]}")
            db.session.commit()
        ...
        if level_achievements:
            _debug_print(f"[ACHIEVEMENT DEBUG] Awarded {len(level_achievements)} level-specific achievement(s): {[a.code for a in level_achievements]}")
            print(f"[ACHIEVEMENT INFO] Awarded {len(level_achievements)} level-specific achievement(s): {[a.title for a in level_achievements]}")
            db.session.commit()
        else:
    ...
    @staticmethod
    @log_query
    def check_all_achievements(user: User, metrics: dict[str, Any]) -> list[Achievement]:
        ...
        for achievement_code, config in achievement_configs.items():
            ...
            # Check question_count achievements (milestones)
            ...
            # Check speed achievements
                ...
                min_questions = requirements.get("min_questions", 0)
                meets_requirements = avg_speed > 0 and avg_speed <= max_speed and total_answers >= min_questions
            
            # Check streak achievements
            elif req_type == "streak":
                min_streak = requirements.get("min_streak_days", 0)
                meets_requirements = current_streak >= min_streak
    ...
    def check_level_specific_achievements(user: User) -> list[Achievement]:
        ...
        for achievement_code, config in achievement_configs.items():
            ...
            # Check level_accuracy achievements
                ...
                _debug_print(f"[ACHIEVEMENT DEBUG]   level_accuracy: level={level}, required_accuracy={min_accuracy}, required_questions={min_questions}")
            ...
            # Check fast_session achievements
            elif req_type == "fast_session":
                max_avg_time = requirements.get("max_avg_time", 5.0)
                min_questions = requirements.get("min_questions", 10)
                
                # Find sessions meeting the speed requirement
                sessions = (
                    PracticeSession.query.filter_by(user_id=user.id)
                    .filter(PracticeSession.completed_at.isnot(None))
                    .all()
                ...
                for session in sessions:
                    if session.total_questions >= min_questions:
                        total_duration_ms = session.total_duration_ms or 0
                        avg_time = (total_duration_ms / 1000.0 / session.total_questions) if session.total_questions > 0 else None
                        if avg_time and avg_time < max_avg_time:
                            _debug_print(f"[ACHIEVEMENT DEBUG]   ✓ AWARDING {achievement_code} (fast_session: {avg_time:.2f}s < {max_avg_time}s)")
                            achievement = AchievementService.create_achievement(
                                user_id=user.id,
                                code=achievement_code,
                                title=config["title"],
            ...
            # Check fast_questions achievements
            elif req_type == "fast_questions":
                max_avg_time = requirements.get("max_avg_time", 4.0)
                consecutive_count = requirements.get("consecutive_count", 10)
                ...
                if len(recent_responses) >= consecutive_count:
                    # Calculate average time for these responses
                    total_time = sum(r.duration_ms or 0 for r in recent_responses)
                    avg_time = (total_time / 1000.0 / len(recent_responses)) if recent_responses else None
                    
                    if avg_time and avg_time < max_avg_time:
                        _debug_print(f"[ACHIEVEMENT DEBUG]   ✓ AWARDING {achievement_code} (fast_questions: {avg_time:.2f}s < {max_avg_time}s for {consecutive_count} questions)")
                        achievement = AchievementService.create_achievement(
                            user_id=user.id,
                            code=achievement_code,
                            title=config["title"],
