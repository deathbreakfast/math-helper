...
def test_achievement_service_001_create_achievement_without_session_id(app, test_user):
...
def test_achievement_service_002_create_achievement_with_session_id(app, test_user, test_session):
...
def test_achievement_service_003_check_test_tier_achievements_passes_session_id(app, test_user, test_session):
...
def test_achievement_service_004_query_achievements_by_session(app, test_user, test_session):
...
def test_achievement_service_005_serialize_achievement_includes_session_id(app, test_user, test_session):
    ...
    with app.app_context():
        ...
        assert serialized["sessionId"] == test_session.id
        assert serialized["code"] == "test-serialization"


def test_achievement_service_006_serialize_achievement_without_session_id(app, test_user):
...
def test_achievement_service_007_test_tier_achievement_rank_b(app, test_user):
...
def test_achievement_service_008_test_tier_achievement_rank_a(app, test_user):
    ...
    with app.app_context():
        ...
        # Should award Rank A achievement
        assert len(achievements) == 1
        achievement = achievements[0]
        assert achievement.session_id == session.id
        assert achievement.code.endswith("-a")
        assert "addition-1digit" in achievement.code
