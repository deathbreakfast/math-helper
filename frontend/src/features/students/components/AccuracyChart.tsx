...
const AccuracyChart = ({ user, filterLevel, onFilterChange }: Props) => {
  const accuracyHistory = useMemo(() => generateAccuracyHistory(user), [user])
      ...
      <div className="h-72">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={accuracyHistory}>
            ...
            />
            <Tooltip content={<CustomTooltip formatValue={(value) => `${Math.round(value)}%`} />} />
            <Legend
              wrapperStyle={{ fontSize: 12, paddingTop: 12 }}
              iconType="plainline"
              verticalAlign="bottom"
            />
            {accuracySeries.map((series, index) => (
              <Line
                key={series.key}
                type="monotone"
                dataKey={series.key}
