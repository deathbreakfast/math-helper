...
export const ForceGraphCanvas: React.FC<ForceGraphCanvasProps> = ({
  ...
  const nodePaint = useCallback((node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    const size = forceNode.size || 35
    const radius = size / 2

    // Determine colors and gradients based on status
    ...
    if (forceNode.title) {
      ...
      // Truncate text if too long (max 20 chars)
      const maxWidth = 100
      ...
      ctx.fillText(displayText, node.x, node.y + radius + 8)
      ctx.restore()
    }
  }, [])
  ...
  const linkPaint = useCallback((link: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    const arrowLength = 8
    const arrowWidth = 5
    const arrowX = target.x - Math.cos(angle) * (target.size || 35) / 2
    const arrowY = target.y - Math.sin(angle) * (target.size || 35) / 2
  ...
  return (
    ...
    >
      <ForceGraph2D
        ...
        nodeRelSize={4}
        nodeVal={(node: any) => (node as ForceGraphNode).size || 35}
        linkDistance={180}
        linkStrength={0.6}
        chargeStrength={-400}
        cooldownTicks={100}
        ...
        nodeLabel={(node: any) => {
          const forceNode = node as ForceGraphNode
          return `${forceNode.title}\n${forceNode.tier} - ${forceNode.status}`
        }}
