...
type AddLearnerModalProps = {
  ...
  onAddUser: (user?: { name: string; avatar: string; pin: string }) => Promise<void>
...
const AddLearnerModal = ({
  ...
  useEffect(() => {
    ...
    // Only sync when modal first opens, not on every render
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen])

  const titleId = 'add-learner-title'
  ...
  return (
    ...
    >
      <div className="max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between" data-testid="testid-add-learner-modal">
          <button
            type="button"
            onClick={handleBack}
